...
class App extends Component {
	...
	loadData = async () => {
		...
		try {
			const res = await fetch("https://cors-anywhere.herokuApp.com/https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?limit=50", settings);
			...
			for (let i = 0; i < value.length; i++) {
				identifiers.push(value[i].id)
			}

			const info = await fetch(`https://cors-anywhere.herokuApp.com/https://pro-api.coinmarketcap.com/v1/cryptocurrency/info?id=${identifiers}`, settings);
			const infoResult = await info.json();
			console.log(infoResult)
			//get icons, description, urls from infoResult array
			for (let i in value) {
				value[i].logo = this.getCryptocurrenciesLogoById(value[i].id, infoResult.data);
				value[i].description = this.getCryptocurrenciesDescriptionById(value[i].id, infoResult.data)
				value[i].urls = this.getCryptocurrenciesUrlsById(value[i].id, infoResult.data)
			}
	...
	render() {
		...
		const currentCrypto = cryptocurrencies.slice(indexOfFirstCrypto, indexOfLastCrypto);

		return (
			<div className="container-fluid">
				<h1 className="text-center m-5">Cryptotrack App</h1>
				<Pagination
				...
				/>
				<CryptoGrid
					cryptocurrencies={currentCrypto}
				/>
				<Pagination
					cryptoPerPage={cryptoPerPage}
					totalCrypto={cryptocurrencies.length}
